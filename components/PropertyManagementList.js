    ...
    return (
      ...
      >
        ...
        <div className="w-2/5 min-w-0 pr-4 relative flex-shrink-0 overflow-hidden">
          {isPaid && (
            <span className="absolute -left-1.5 top-1/2 -translate-y-1/2 text-amber-500 text-[10px] leading-none z-10">⭐</span>
          )}
          <div className={`truncate ${isPaid ? 'pl-3' : ''}`} title={pm.company_name || pm.name || "Property Manager"}>
        ...
        <div className="flex-1 text-xs min-w-0 pl-4 overflow-hidden flex items-center justify-center flex-shrink-0">
          <div className="truncate overflow-hidden" title={pm.unit_count !== undefined ? pm.unit_count?.toString() : "—"}>
            {pm.unit_count !== undefined ? pm.unit_count : "—"}
          </div>
        </div>
      </div>
    );
